...
class Bottle2neck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1, downsample=None, baseWidth=26, scale=4, stype='normal'):
        ...
        self.bn1 = nn.GroupNorm(
            next(int(width * scale/i)for i in range(16,0,-1)if width * scale%i==0), 
            num_channels=width * scale
        )
        ...
        for i in range(self.nums):
            ...
            bns.append(nn.GroupNorm(
                next(int(width/i)for i in range(16,0,-1)if width%i==0), 
                num_channels=width
            ))
        ...
        self.bn3 = nn.GroupNorm(
            next(int(planes * self.expansion/i)for i in range(16,0,-1)if planes * self.expansion%i==0), 
...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, n_channels=1, baseWidth=26, scale=4, num_classes=1000, output_stride=32):
        self.inplanes = 64
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv2d(n_channels, 32, 3, 2, 1, bias=False),
            # nn.BatchNorm2d(32),
            nn.GroupNorm(
                next(int(32/i)for i in range(16,0,-1)if 32%i==0), 
                num_channels=32
            ),
        ...
        self.bn1 = nn.GroupNorm(
            next(int(64/i) for i in range(16,0,-1) if 64%i==0), 
            num_channels=64
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                nn.GroupNorm(
                    next(int(planes * block.expansion/i)for i in range(16,0,-1)if planes * block.expansion%i==0), 
...
def res2net50_v1b_26w_4s(pretrained=False, n_channels=1, **kwargs):
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4, n_channels=n_channels, **kwargs)
    if pretrained:
